...
class Resident(BaseAgent):
    ...
    @classmethod
    def generate_agent_properties(cls, demographics, parish_demographics, parish, city, 
                                 industry_distribution, occupation_distribution, income_distribution, 
                                 random_generator):
        """
        ...
        Args:
            demographics: Global demographic data
            parish_demographics: Parish-specific demographic data
            parish: The parish name to use for demographics (optional)
            city: City name for location-specific features
        ...
        if parish and parish in parish_demographics:
            demo_data = parish_demographics[parish]
        ...
        # --- AGE & AGE CLASS ---
        age_dist = demo_data.get('age_distribution', {})
        age_group = cls._sample_from_distribution(age_dist, random_generator)
        ...
        if age_group is not None:
            if '-' in age_group:  # e.g. "25-29"
                ...
                try:
                    min_age = int(min_age)
                    max_age = int(max_age)
                    props['age'] = random_generator.randint(min_age, max_age)
                except ValueError:
                    # Fallback in case parsing fails
                    props['age'] = random_generator.randint(18, 90)
            elif age_group.endswith('+'):  # e.g. "85+"
                try:
                    min_age = int(age_group.rstrip('+'))
                    props['age'] = random_generator.randint(min_age, min_age + 10)
                except ValueError:
                    props['age'] = random_generator.randint(65, 90)
        ...
        if demo_data.get('gender_by_age') and age_group in demo_data['gender_by_age']:
            gender_dist = demo_data['gender_by_age'][age_group]
        else:
            gender_dist = demo_data.get('gender_distribution', {})
        props['gender'] = cls._sample_from_distribution(gender_dist, random_generator)

        # --- INCOME ---
        income_dist = demo_data.get('income_distribution', {})
        income_level = cls._sample_from_distribution(income_dist, random_generator)

        # Convert income level to actual income - use parish-specific ranges if available
        income_ranges = demo_data.get('income_ranges', {
            "low": (10000, 30000),
            "medium": (30001, 100000),
            "high": (100001, 500000)
        })
        ...
        if income_level in income_ranges:
            ...
            props['income'] = random_generator.randint(min_val, max_val)
        else:
            # Fallback to default ranges
            if income_level == "low":
                props['income'] = random_generator.randint(10000, 30000)
            elif income_level == "medium":
                props['income'] = random_generator.randint(30001, 100000)
            else:  # high
                props['income'] = random_generator.randint(100001, 500000)

        # Note: For Macau, income will be reassigned based on occupation after occupation is determined

        # --- EDUCATION LEVEL ---
        education_level = None
        if (demo_data.get('education_distribution_by_age_and_gender') and
            age_group in demo_data['education_distribution_by_age_and_gender'] and
            props['gender'] in demo_data['education_distribution_by_age_and_gender'][age_group]):
            edu_dist_raw = demo_data['education_distribution_by_age_and_gender'][age_group][props['gender']]
            ...
            education_level = cls._sample_from_distribution(flattened_edu_dist, random_generator)
        else:
            # Fallback to generic education distribution if provided
            education_dist = demo_data.get('education_distribution', {})
            education_level = cls._sample_from_distribution(education_dist, random_generator)
        props['education'] = education_level

        # --- INDUSTRY ---  
        industry_choice = None
        if (city == "Macau, China" and education_level and industry_distribution):
            norm_edu = cls._normalise_string(education_level)
            ...
            if not industry_dist:
                ...
                for key, dist in industry_distribution.items():
                    if key in norm_edu or norm_edu in key:
                        ...
                        break
            if industry_dist:
                industry_choice = cls._sample_from_distribution(industry_dist, random_generator)
        ...
        if (city == "Macau, China" and props.get('age') is not None and occupation_distribution):
            ...
            if age_group_occ and age_group_occ in occupation_distribution:
                occupation_dist = occupation_distribution[age_group_occ]
                occupation_choice = cls._sample_from_distribution(occupation_dist, random_generator)
        ...
        if (city == "Macau, China" and occupation_choice and income_distribution):
            ...
            if income_key in income_distribution:
                ...
                income_bracket = cls._sample_from_distribution(income_bracket_dist, random_generator)
                
                # Convert income bracket to actual income value
                if income_bracket:
                    actual_income = cls._convert_income_bracket_to_value(income_bracket, random_generator)
    ...
    @staticmethod
    def _sample_from_distribution(distribution, random_generator):
        """
        ...
        return random_generator.choices(items, probabilities)[0]
    
    @staticmethod
    def _normalise_string(s):
        """Normalize string for matching (lowercase, stripped of special characters)"""
        return re.sub(r"[^a-z0-9]", "", str(s).lower()) if s else ""

    @staticmethod
    def _convert_income_bracket_to_value(income_bracket, random_generator):
        """
        ...
        if "≧" in income_bracket:  # e.g., "≧60 000"
            ...
            return random_generator.randint(min_income, min_income + 40000)
            ...
            return random_generator.randint(max(1000, max_income - 1000), max_income - 1)
            ...
            if len(parts) == 2:
                ...
                return random_generator.randint(min_income, max_income)
